...
import { Plus, Minus, MousePointer2, Hand } from 'lucide-react';
...
export default function CanvasContainer() {
  ...
  const [widgets, setWidgets] = useState<WidgetType[]>(INITIAL_WIDGETS);
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-gray-900 select-none">
      {/* 툴바 UI */}
      <div className="absolute top-4 left-4 z-50 flex gap-2 rounded-lg border bg-white p-2 shadow-md">
        <div className="flex items-center gap-2 px-2 text-sm font-bold text-gray-600">
      ...
      >
        <div
          ...
          style={{
            ...
            // 점 사이의 간격 (줌 레벨에 따라 간격도 늘어나야 함)
            backgroundSize: `${20 * camera.z}px ${20 * camera.z}px`,

            // 패턴의 시작 위치 (카메라 이동에 따라 패턴도 밀려야 함)
            // 공식: (카메라위치 % 한칸크기) -> 이렇게 하면 무한 반복됨
        ...
        >
          ...
          {widgets.map(widget => (
            <div
              ...
              className={`absolute top-0 left-0 p-4 rounded-lg shadow-lg cursor-grab active:cursor-grabbing border border-black/10 transition-shadow ${widget.color}`}
            >
              <div className="font-bold mb-2 flex justify-between items-center text-gray-700">
                <span>{widget.type === 'note' ? 'Sticky Note' : 'Task Card'}</span>
                <Hand size={14} />
              </div>
              ...
              {/* 공동 편집자 커서 예시 (가짜) */}
          {/* {widget.id === '1' && (
                <div className="absolute -top-3 -right-3 bg-blue-500 text-white text-[10px] px-2 py-0.5 rounded-full border border-white flex items-center gap-1 shadow-sm">
                  User B editing...
                </div>
              )}
            </div> */}
